...
export default function AnalyticsPage() {
  return (
    <div className="space-y-6 animate-fade-up">
      <div>
        <h2 className="text-2xl font-bold mb-1 text-left">Safety Analytics</h2>
        <p className="text-[var(--color-text-muted)] text-sm text-left">System-wide trends and heat exposure metrics</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="h-80 flex flex-col p-6" hoverable={false}>
          <h3 className="font-semibold mb-6 flex items-center gap-2 text-left">
          ...
          <div className="flex-1 border-b border-l border-[var(--color-border)] relative flex items-end justify-between px-4 pb-4 pt-10 mt-auto">
            ...
            {[45, 48, 52, 54, 50, 47, 44].map((val, i) => (
              <div key={i} className="relative group w-8 sm:w-12">
                <div 
                  className="w-full rounded-t-sm transition-all duration-500 hover:opacity-80 animate-fade-up"
                  style={{ 
                    height: `${(val / 60) * 100}%`,
                    background: val > 50 ? 'linear-gradient(to top, #ef4444, #6366F1)' : 'linear-gradient(to top, #6366F1, #818CF8)'
                  }}
                />
                <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs text-[var(--color-text-muted)] font-semibold">
                  {['M', 'T', 'W', 'T', 'F', 'S', 'S'][i]}
                </span>
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/80 px-2 py-1 rounded text-xs text-white">
                  {val}°C
                </div>
              </div>
            ))}
